-- Retrying the same bearer token by the account that already consumed it is
-- successful. Other accounts still receive the same fail-closed response.
...
begin
  ...
  if invite_row.state = 'consumed' and invite_row.consumed_by = actor_id then
    select relation.id into relation_id
    from core.guardian_relations relation
    where relation.club_id = invite_row.club_id
      and relation.guardian_person_id = invite_row.guardian_person_id
      and relation.child_person_id = invite_row.child_person_id
      and relation.created_by = actor_id
      and relation.state = 'active'
    order by relation.starts_at desc, relation.id desc
    limit 1;
  ...
  if invite_row.state <> 'issued' then
    raise invalid_parameter_value using message = 'invalid_or_expired_token';
  end if;
  ...
  if linked_profile_id is null then
    ...
    insert into core.person_account_links(
      ...
      invite_row.club_id, invite_row.guardian_person_id, actor_id,
      'active', now(), actor_id
    );
  end if;

  insert into core.guardian_relations(
    club_id, guardian_person_id, child_person_id, kind, state, starts_at,
    created_by
  ) values (
    invite_row.club_id, invite_row.guardian_person_id,
    invite_row.child_person_id, 'guardian', 'active', now(), actor_id
  ) returning id into relation_id;

  update core.guardian_invites
  set state = 'consumed', consumed_at = now(), consumed_by = actor_id,
      revision = revision + 1
  ...
  insert into internal.command_deduplication(
    ...
    actor_id, idempotency_key, 'roster.guardian_invite.accept.v1',
  ...
  insert into audit.command_events(
    ...
    invite_row.club_id, actor_id, 'roster.guardian_invite.accept.v1',
    'guardian_relation', relation_id, 1,
    jsonb_build_object(
...
insert into internal.migration_provenance(
  ...
  '20260902072501_team05_guardian_claim_retry_safe',
  'greenfield',
  'TEAM-05 same-actor guardian bearer-token retries return active relation'
);
