-- The team-wide statistics migration (20260907093000) rewrote
-- internal.get_main_surfaces_for_actor from the *original* S05 migration's
-- text, which unintentionally undid 20260808095006_s05_fix_club_person_status
-- (core.club_people's column is `status`, not `state`) since that fix was
-- applied as a dynamic text-replace against the live function rather than a
-- migration that's visible by reading the CREATE-time source. Confirmed via
-- direct query against the hosted project: `core.club_people` has no
-- `state` column. Re-applying the same fix here.

create or replace function internal.get_main_surfaces_for_actor(requested_context_ids uuid[])
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  with selected_contexts as materialized (
  ...
  ), manageable_contexts as materialized (
    select distinct context.club_id, context.team_id
    from selected_contexts context
    where context.team_id is not null
      and (
        'event.manage' = any(context.capabilities)
        or 'event.attendance.manage' = any(context.capabilities)
      )
  ), attendance as materialized (
    select fact.status, fact.revision
    from manageable_contexts managed
    join core.event_teams relation on relation.team_id = managed.team_id
      and relation.club_id = managed.club_id
    join core.attendance_facts fact on fact.event_id = relation.event_id
      and fact.club_id = relation.club_id
    ...
    join visible_events event_row on event_row.id = fact.event_id
    where not exists (select 1 from manageable_contexts)
  ), action_rows as (
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'member_count', case when context.team_id is null then
        (select count(*) from core.club_people person where person.club_id = context.club_id and person.status = 'active')
      else
