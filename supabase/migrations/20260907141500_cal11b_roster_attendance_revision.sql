-- CAL-11 follow-up: the roster's attendance recording needs each person's
-- current attendance_facts revision (0 if none exists yet) to build a
-- correct expected_revision per change in record_attendance — missed in
-- the previous migration. Forward fix, not an edit of the already-applied
-- one (it's already live).
...
values('20260907141500_cal11b_roster_attendance_revision','greenfield','CAL-11 follow-up — roster needs attendance revision for record_attendance changes');
