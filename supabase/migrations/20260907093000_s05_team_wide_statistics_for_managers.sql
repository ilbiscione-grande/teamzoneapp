-- Physical walkthrough finding: the "statistics" projection only ever
-- counted the *actor's own* attendance_facts row, never the team's. A
-- leader/club_functionary isn't usually a roster player themselves, so
-- Statistik showed "no attendance yet" no matter how much real attendance
-- got recorded for their team. Contexts with event.manage or
-- event.attendance.manage (i.e. anyone who can actually record/manage
-- attendance) now see team-wide totals across their team's events instead;
-- everyone else (players/guardians without that capability) keeps seeing
-- their own attendance, unchanged.
