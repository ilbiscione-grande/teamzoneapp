-- CAL-11 follow-up: the Deltagare tab's "Påminn" action needs the same
-- pending/expiry/6h-cooldown facts the server's own remind_callup_for_actor
-- already enforces, so the client can disable the action proactively
-- instead of letting a doomed-to-fail tap round-trip to the server first.
-- Comparing against the reference implementation's older Teamzone project
-- (C:/Dev/Teamzone) surfaced this gap.
...
values('20260907150000_cal11c_roster_reminder_fields','greenfield','CAL-11 follow-up — roster carries callup expiry/last-reminded for client-side reminder cooldown gating');
