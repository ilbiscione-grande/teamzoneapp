...
export async function GET(request: Request, context: { params: Promise<{ teamKey: string; teamSlug: string }> }) {
  try {
    const { teamKey, teamSlug } = await context.params;
    const { config, ipHash } = requestIpHash(request);
    const data = await publicRpc(createServerSupabase(config), "public_get_team", {
      club_slug: teamKey, team_slug: teamSlug, ip_hash: ipHash,
    });
