...
- [x] Före eventet: kryssruta väljer utkastmedlemskap. Efter utskickade kallelser: kallelsestatus visas i stället (med påminn/återkalla där behörighet finns). Efter eventets slut: närvarostatus väljs inline för kallade rader.
...
- [x] Nytt widgettest kör hela flödet (öppna sida, statusrad, hinksortering, sök och lägg till klubbövergripande kandidat).

**Medvetet avgränsat:** grupper (spara/återanvända spelarurval) är en separat, ännu obyggd funktion — inget backend-stöd finns; användarens egna produktbeslut var att göra sida/statusrad/deltagarflik i ett svep och grupper som ett senare steg.
